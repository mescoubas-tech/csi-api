...
def _normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # construire agent_id si manquant à partir de nom/prenom
...
def _merge_pdf_tables(tables: List[pd.DataFrame]) -> pd.DataFrame:
    ...
    for raw in tables:
        if raw is None or raw.empty:
            continue
        df = raw.copy()
        # utilise la 1re ligne comme header si possible
        try:
...
def _read_pdf_tables(file_bytes: bytes) -> pd.DataFrame:
    """Extrait des tableaux d’un PDF numérique via pdfplumber (pas d’OCR)."""
    ...
    if not tables:
        raise ValueError("Aucun tableau détecté dans le PDF (fichier probablement scanné/image).")
